...
export function EmployeeListClient({ employees }: { employees: any[] }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* High Contrast Corporate Employees Data Table */}
      {employees.length === 0 ? (
                  ...
                  <tr key={emp.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="p-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-md bg-purple-100 dark:bg-purple-950/80 text-purple-800 dark:text-purple-200 border border-purple-200 dark:border-purple-800 flex items-center justify-center font-bold text-xs">
                          {emp.name.charAt(0)}
                        </div>
                        <div>
                          <p className="font-bold text-slate-900 dark:text-white text-xs">{emp.name}</p>
                          {emp.email && <p className="text-[11px] font-semibold text-slate-500 dark:text-slate-400 font-mono" dir="ltr">{emp.email}</p>}
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Salary */}
                    <td className="p-3.5 font-extrabold text-purple-700 dark:text-purple-300">
                      {formatCurrency(emp.salary, emp.currency)}
                    </td>

                    {/* Phone */}
                    <td className="p-3.5 text-slate-600 dark:text-slate-400 font-mono font-semibold" dir="ltr">
                      {emp.phone || '—'}
                    </td>

                    {/* Invoices Count */}
                    <td className="p-3.5 text-center font-semibold">
                      <span className="px-2.5 py-1 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-md text-[11px] font-bold text-slate-800 dark:text-slate-200">
